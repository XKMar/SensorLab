...
module.exports = function(root, esc, webURL) {
  ...
  function render() {return `<div class="publication-intro"><span class="eyebrow">EXPLORE BY TOPIC</span><p>按研究方向浏览代表性论文与在研主题</p></div><nav class="publication-categories" aria-label="按研究方向浏览">${categories.map(([id,label,desc])=>{const count=items.filter(p=>p.category===id).length;return `<a class="publication-category" href="#pub-${id}"><span class="category-top"><span class="category-index">${id.toUpperCase()}</span><span class="category-arrow" aria-hidden="true">↗</span></span><strong>${label}</strong><span class="category-desc">${desc}</span><span class="category-count">${count?`${count} 篇论文`:'在研方向'}</span></a>`}).join('')}</nav>${categories.map(([id,label,desc])=>`<section class="publication-group" id="pub-${id}">${id==='motion'?'<span id="pub-gait"></span><span id="pub-adaptation"></span><span id="pub-multimodal"></span>':''}<div class="publication-group-head"><h2>${label}</h2><p>${desc}</p></div>${items.filter(p=>p.category===id).sort((a,b)=>b.year-a.year).map(row).join('')}${id==='world'?'<p class="publication-legend">该方向正在探索中。<a href="research/world-models.html">了解研究方向 ›</a></p>':''}</section>`).join('')}`;}
