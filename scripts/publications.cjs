...
module.exports = function(root, esc, webURL) {
  ...
  function row(p) {
    ...
    return `<article class="publication" id="paper-${p.id}"><div class="publication-visual"><span class="venue-badge ${badgeClass}">${esc(p.badge)}</span><a href="${esc(p.url)}" target="_blank" rel="noopener" aria-label="查看 ${esc(p.title)} 论文"><img src="${esc(p.image)}" alt="${esc(p.title)}：${esc(p.image_alt||'论文图示')}" loading="lazy" width="360" height="140"></a></div><div class="publication-info"><h3><a href="${esc(p.url)}" target="_blank" rel="noopener noreferrer">${esc(p.title)}</a></h3><p class="publication-authors">${authors}</p><p class="publication-venue"><em>${esc(p.venue)}</em>, ${p.year}${p.volume?` · ${esc(p.volume)}${p.number?'('+esc(p.number)+')':''}`:''}${p.pages?' · '+esc(p.pages.replaceAll('--','–')):''}</p><div class="publication-actions"><details class="bib-details"><summary aria-label="展开 ${esc(p.title)} 的 BibTeX">BIB</summary><div class="bib-panel"><pre><code>${esc(bib(p))}</code></pre><a href="assets/bib/${p.id}.bib" download>下载 .bib</a></div></details>${p.pdf?`<a class="paper-button" href="${esc(p.pdf)}" target="_blank" rel="noopener noreferrer" title="${esc(p.pdf_note||'阅读论文 PDF')}">Paper</a>`:''}<a class="paper-button" href="${esc(p.code || p.pdf || p.url)}" target="_blank" rel="noopener noreferrer" title="${p.code?'查看代码':'暂无代码，查看论文 PDF'}">CODE</a></div></div></article>`;
  }
  // Small, familiar line icons keep navigation distinct from the papers below.
  const topicIcons = {
    motion: '<circle cx="13" cy="4" r="2"/><path d="m8 11 3-4 4 3 4 1M11 7l-1 7-5 6m5-6 5 2 2 5M8 11l-4 1"/>',
    localization: '<path d="M20 10c0 6-8 12-8 12S4 16 4 10a8 8 0 1 1 16 0Z"/><circle cx="12" cy="10" r="3"/>',
    physiology: '<path d="M2 12h4l3-8 5 16 3-8h5"/>',
    world: '<rect x="3" y="3" width="6" height="6" rx="1"/><rect x="15" y="15" width="6" height="6" rx="1"/><path d="M12 6h6v6m-3-3 3 3 3-3M12 18H6v-6m3 3-3-3-3 3"/>'
  };
  const topicSummaries = {
    motion: '步态识别 · 运动表征',
    localization: '卫星 · 无人机 · 地面',
    physiology: '生理测量 · 认知状态',
    world: '动作预测 · 环境变化'
  };
  function render() {return `<div class="publication-intro"><span class="eyebrow">EXPLORE BY TOPIC</span><p>按研究方向浏览代表性论文与在研主题</p></div><nav class="publication-categories" aria-label="按研究方向浏览">${categories.map(([id,label,desc])=>{const count=items.filter(p=>p.category===id).length;return `<a class="publication-category" href="#pub-${id}"><span class="category-icon" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">${topicIcons[id]}</svg></span><span class="category-copy"><strong>${label}</strong><span class="category-desc">${topicSummaries[id]}</span><span class="category-count">${count?`${count} 篇论文`:'在研方向'}</span></span></a>`}).join('')}</nav>${categories.map(([id,label,desc])=>`<section class="publication-group" id="pub-${id}">${id==='motion'?'<span id="pub-gait"></span><span id="pub-adaptation"></span><span id="pub-multimodal"></span>':''}<div class="publication-group-head"><h2>${label}</h2><p>${desc}</p></div>${items.filter(p=>p.category===id).sort((a,b)=>b.year-a.year).map(row).join('')}${id==='world'?'<p class="publication-legend">该方向正在探索中。<a href="research/world-models.html">了解研究方向 ›</a></p>':''}</section>`).join('')}`;}
